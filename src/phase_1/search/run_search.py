...
def run_experiment():
    configs = [
        ...
        ("1-B Standard+Heuristic", False, "heuristic", 3),
        ("1-C Afterstate+Heuristic", True, "heuristic", 3),
        ("1-D Standard+StateNTuple", False, "ntuple_state", 3),
        ("1-E Standard+AfterstateNTuple", False, "ntuple_afterstate", 3),
        ("1-F Afterstate+StateNTuple", True, "ntuple_state", 3),
        ("1-G Afterstate+AfterstateNTuple", True, "ntuple_afterstate", 3)
    ]
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        ...
        for name, use_aft, eval_type, depth in configs:
            ...
            print(row_str)
            f.write(row_str + "\n")
            f.flush() # 强制刷新缓冲，确保每一行跑完立马写进硬盘防崩溃
            
        print("="*145 + "\n")

if __name__ == "__main__":
    multiprocessing.freeze_support() # Windows 必须加这句
    run_experiment()
...
class MCTSAgent:
    ...
    def _simulate(self, node: Node, current_raw: int, depth: int) -> float:
        ...
        if not node.is_chance:
            ...
            # ==============================================================
            # 【用户制定的规则 1：混合一定基础概率的先验，既有区分度又能全面探索】
            # ==============================================================
            ...
            # ==============================================================
            # 【用户制定的规则 2：非零局部保底的 FPU 机制】
            # 如果当前节点为0，那么填充至已探索的“最小非零真实期望”
            # 如果全为0(尚未有兄弟被探索)，默认赋予 0.0，靠上面的 Prior 保底探索
            # ==============================================================
            visited_qs = []
            for a in legal_actions:
            ...
            for a in legal_actions:
                ...
                val_score = self.min_max_stats.normalize(actual_val)

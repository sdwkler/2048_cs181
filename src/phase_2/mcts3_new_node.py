# 文件路径: src/phase_2/mcts3_new_node.py
...
# ==========================================================
# 2. 核心架构: Stochastic MuZero 二分严格树 (增加环境感知与深度探针)
# ==========================================================
class MinMaxStats:
    def __init__(self):
...
class MCTSAgent:
    # 【神级修改 1】：增加 p4_prob 环境感知参数
    def __init__(self, use_afterstate: bool = False, exploration_c: float = 2000.0, seed: int | None = None, max_tree_depth: int = 64, rollout_limit: int = 5, p4_prob: float = 0.1):
        ...
        self.current_max_depth = 0 # 记录单次决策的搜索树最深抵达层数

    # 【神级修改 2】：返回增加 int 类型的深度探针
    def get_best_action(self, b: board, num_simulations: int = 100) -> tuple[int, float, float, int]:
        ...
        self.current_max_depth = 0  # 每次启动搜索前清零深度探针
    ...
    def _simulate(self, node: Node, current_raw: int, depth: int) -> float:
        self.current_max_depth = max(self.current_max_depth, depth) # 更新树的极限深度
    ...
    # 【神级修改 3】：所有的树机会节点和 Rollout 生成，均由漂移概率 p4_prob 控制！

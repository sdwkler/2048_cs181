...
class MCTSAgent:
    ...
    def _simulate(self, node: Node, current_raw: int, depth: int) -> float:
        ...
        if not node.is_chance:
            ...
            if visited_qs:
                local_max = max(visited_qs)
                local_min = min(visited_qs)
            else:
                # 初始没有节点被探索时，保底基线为 0
                local_max, local_min = 0.0, 0.0
                
            safe_range = max(local_max - local_min, 200.0)

            # --- 3. PUCT 动作选择 (完美融入你的 FPU 逻辑) ---
            ...
            for a in legal_actions:
                ...
                if child.visit_count == 0:
                    # 【核心突破：FPU 保底机制】
                    # 正如你所建议的：如果未探索，它的实际期望值隐式等同于当前的 local_min。
                    # 那么归一化后，它的得分必定是 (local_min - local_min) / safe_range = 0.0！
                    # 因为有 prior 在旁边制衡，所以它绝不会陷入“雨露均沾”的盲目探索。
                    val_score = 0.0 
                else:
                    # 对于已探索的节点，正常计算其对于 local_min 的相对优势
                    reward, _ = self._apply_move_cached(current_raw, a)
    ...
    def export_tree_topology(self, max_export_depth=3) -> Dict[str, Any]:
        action_names = {0: "上", 1: "右", 2: "下", 3: "左"}
        def _walk(node: Node, current_depth: int, edge_label: str) -> Dict[str, Any]:
            if current_depth > max_export_depth or not node: return None
            children_list = []
            for action_or_raw, child in node.children.items():
                if node.is_chance:
                    child_label = f"发牌环境分支"
                else:
                    child_label = action_names.get(action_or_raw, f"动作 {action_or_raw}")
                    
                child_data = _walk(child, current_depth + (0 if node.is_chance else 1), child_label)
                if child_data: children_list.append(child_data)
                
            return {
                "label": edge_label, "type": "chance" if node.is_chance else "decision",
                "visits": node.visit_count, "value": round(node.value(), 1), "prior": round(node.prior, 2),
                "children": sorted(children_list, key=lambda x: x['visits'], reverse=True)
            }
        return _walk(self.root, 0, "根节点")